          ...
          headers: {'Authorization': User.getToken()},
            data: {
                item: {
                    ...
                    description: $("#item_desc_add").val(),
                    product_url: $("item_url_add").val()
                }
            }
        }).success(function successCallback(response) {
...
gentooApp.controller('EditController', ['$rootScope', '$scope', '$http', '$location', 'User', function($rootScope, $scope, $http, $location, User) {
    ...
    $scope.deleteFavorite = function(attribute) {
        console.log("This is the attribute of this thing " +attribute);
        var string = attribute
        console.log("What to send to delete:" + string);
        $http({
            method: 'DELETE',
